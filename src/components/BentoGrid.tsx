...
const CardIcon = ({ icon }: { icon: string }) => {
  const iconMap: Record<string, JSX.Element> = {
    ...
    "/**/": <span className="text-4xl font-bold text-[hsl(65,100%,50%)]">/**/</span>,
  };
...
const getCardStyle = (card: CardData) => {
  if (card.bgColor) {
    if (card.bgColor.startsWith("linear-gradient") || card.bgColor.startsWith("radial-gradient")) {
  ...
  const themeStyles: Record<string, React.CSSProperties> = {
    ...
    "gradient-purple": { background: "linear-gradient(135deg, hsl(262,80%,50%), hsl(280,70%,55%))" },
    "gradient-instagram": { background: "linear-gradient(135deg, hsl(35,95%,55%), hsl(330,80%,55%), hsl(280,70%,55%))" },
    "gradient-green": { background: "linear-gradient(135deg, hsl(140,70%,42%), hsl(160,60%,45%))" },
    light: { backgroundColor: "hsl(0,0%,96%)" },
    custom: {},
  };

  return themeStyles[card.theme] || {};
};
...
const BentoCard = ({ card }: { card: CardData }) => {
  ...
  const textColor = isLightTheme(card) ? "text-foreground" : "text-primary-foreground";

  return (
    <div
      onClick={handleClick}
      className={cn(
        "group relative flex cursor-pointer flex-col justify-end overflow-hidden rounded-2xl p-5 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl",
        card.colSpan === 2 ? "col-span-2" : "col-span-1",
        card.rowSpan === 2 ? "row-span-2" : "row-span-1",
        card.rowSpan === 2 ? "min-h-[240px]" : "min-h-[120px]",
      )}
      style={getCardStyle(card)}
    >
      {card.icon && (
        <div className={cn("mb-auto", textColor)}>
          <CardIcon icon={card.icon} />
        </div>
      )}

      {(card.title || card.description) && (
        <div className={cn("mt-auto", textColor)}>
          {card.title && (
